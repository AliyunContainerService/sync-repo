...
def help():
    print('python sync_private_images -h|--help')
    print('python sync_private_images [-f|--file <config_file>] [-r|--registry <host:port>] [-n|--namespace] [-i|--insecure_registry] [-d|--days=15]')
    sys.exit(1)
...
def list_repo_tags(client, repo):
    ...
    if repo_names[0] == 'docker.io':
        ...
        # cmd = "aliyun cr GET  /repos/%s/%s/tags --endpoint cr.cn-hangzhou.aliyuncs.com | jq '.data.tags[].tag'"  % (repo_names[1], repo_names[2])
        manifest = searchTags(cmd, u'manifest')
        for key in manifest:
            image = manifest[key]
...
options = []
DEFAULT_CONFIG_FILE = './private_images.txt'
